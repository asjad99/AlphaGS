...
from logic import *
from action_effect_table import ACTION_EFFECT_HASHTABLE
from condition_action_rules import RULES_DICT

#---------------API call---------------
import json
import socket

def maxSat_APICALL(msg_list):
...
def populate_KB():
    ...
    my_prep_list = ['PP', 'QQ', 'RR', 'SS', 'TT','UU','VV','WW']
    my_symbol_list = ['&', '|', '==>', '<==', '<=>']

    my_KB = PropKB()
    my_KB_confirmed = PropKB()
    ...
    for i in range(0, 330):
        ...
        #Strategy 1: 
        #Pick two random items from first list 
        #with low probability add a not in front of them
        # and add one item from the 2nd list 
        

        # generate random number 0-2
        result = random.sample(range(0, 5), 2)

        # Pick an item
        item1 = my_prep_list[result[0]]
        item2 = my_prep_list[result[1]]
        ...
        symbol = my_symbol_list[random.randint(0, 4)]

        final_clause = str(item1) + ' ' + symbol + ' ' +str(item2) # combine the two

        my_KB.tell(expr(final_clause))

        message2 = {"effect": '[]',#str(my_KB_temp.clauses),
                    "KB": str(my_KB.clauses),
                    "current_state": '[]'
                    }

        result = maxSat_APICALL([message2])

        if result['message'] == 'OK':
                my_KB_confirmed.tell(expr(final_clause))

    message3 = {"effect": '[]',
                "KB": str(my_KB_confirmed.clauses),
                "current_state": '[]'
                }
                

    #result2 = maxSat_APICALL([message3])

    #print("------------  result   -----------------")
    print(my_KB_confirmed.clauses)
    #print(result2)
    #print(my_KB.clauses)
    #print("----------------end function----------")
    #print(my_KB.clauses)

    return my_KB_confirmed
...
class Fig52Game(Game):
    ...
    def __init__(self):
        self.playA_Turn = True
        self.knowledgebase = populate_KB()

    def actions_prev(self, state):
    ...
    def result_prev(self, state, move):
        return self.succs[state][move]

    def result(self,current_state, last_action):
    ...
    def utility(self, current_state):
        ...
        reward = reward / len(current_state)
